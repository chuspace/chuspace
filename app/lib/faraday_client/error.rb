...
module FaradayClient
  ...
  class Error < StandardError
    attr_reader :context
    # Returns the appropriate FaradayClient::Error subclass based
    # on status and response message
    #
    # @param [Hash] response HTTP response
    # @return [FaradayClient::Error]
    def self.from_response(response)
      ...
      if klass =  case status
                  when 400      then FaradayClient::BadRequest
                  when 401      then error_for_401(headers)
                  when 403      then error_for_403(body)
                  when 404      then error_for_404(body)
                  when 405      then FaradayClient::MethodNotAllowed
                  when 406      then FaradayClient::NotAcceptable
                  when 409      then FaradayClient::Conflict
                  when 415      then FaradayClient::UnsupportedMediaType
                  when 422      then error_for_422(body)
                  when 451      then FaradayClient::UnavailableForLegalReasons
                  when 400..499 then FaradayClient::ClientError
                  when 500      then FaradayClient::InternalServerError
                  when 501      then FaradayClient::NotImplemented
                  when 502      then FaradayClient::BadGateway
                  when 503      then FaradayClient::ServiceUnavailable
                  when 500..599 then FaradayClient::ServerError
         end
        klass.new(response)
      end
    end
    ...
    def build_error_context
      if RATE_LIMITED_ERRORS.include?(self.class)
        @context = FaradayClient::RateLimit.from_response(@response)
      end
    end
    ...
    def self.error_for_401(headers)
      if FaradayClient::OneTimePasswordRequired.required_header(headers)
        FaradayClient::OneTimePasswordRequired
      else
        FaradayClient::Unauthorized
      end
    end
    ...
    def self.error_for_403(body)
      if body =~ /rate limit exceeded/i
        FaradayClient::TooManyRequests
      elsif body =~ /login attempts exceeded/i
        FaradayClient::TooManyLoginAttempts
      elsif body =~ /returns blobs up to [0-9]+ MB/i
        FaradayClient::TooLargeContent
      elsif body =~ /abuse/i
        FaradayClient::AbuseDetected
      elsif body =~ /repository access blocked/i
        FaradayClient::RepositoryUnavailable
      elsif body =~ /email address must be verified/i
        FaradayClient::UnverifiedEmail
      elsif body =~ /account was suspended/i
        FaradayClient::AccountSuspended
      elsif body =~ /billing issue/i
        FaradayClient::BillingIssue
      elsif body =~ /Resource protected by organization SAML enforcement/i
        FaradayClient::SAMLProtected
      elsif body =~ /suspended your access|This installation has been suspended/i
        FaradayClient::InstallationSuspended
      else
        FaradayClient::Forbidden
      end
    end
    ...
    def self.error_for_404(body)
      if body =~ /Branch not protected/i
        FaradayClient::BranchNotProtected
      else
        FaradayClient::NotFound
      end
    end
    ...
    def self.error_for_422(body)
      if body =~ /PullRequestReviewComment/i && body =~ /(commit_id|end_commit_oid) is not part of the pull request/i
        FaradayClient::CommitIsNotPartOfPullRequest
      elsif body =~ /Path diff too large/i
        FaradayClient::PathDiffTooLarge
      else
        FaradayClient::UnprocessableEntity
      end
    end
  ...
  RATE_LIMITED_ERRORS = [FaradayClient::TooManyRequests, FaradayClient::AbuseDetected]
end
